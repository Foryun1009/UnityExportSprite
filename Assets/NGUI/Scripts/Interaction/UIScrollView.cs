...
public class UIScrollView : MonoBehaviour
{
    ...
    /// can drag max pos
    /// </summary>
    public Vector2 mCanDragLimitPos = Vector2.zero;

    /// <summary>
    /// the critical value use to Corrected Drag Pos
    /// </summary>
    public int CirticalValue2CorrectedPos = 80;
	...
	public void Drag (Vector2 delta)
	{
        ...
        if (!DragIsCanMoveByLimitPos(delta)) return;
    ...
    //judge is can drag by limit pos
    private bool DragIsCanMoveByLimitPos(Vector2 delta) {
        if (mCanDragLimitPos == Vector2.zero) return true;

        if (movement == Movement.Horizontal)
        {
            if (transform.localPosition.x < mCanDragLimitPos.x)
            {
                float xOffset = mCanDragLimitPos.x - transform.localPosition.x;
                if (xOffset > CirticalValue2CorrectedPos)
                {
                    Vector3 pos = new Vector3(mCanDragLimitPos.x, transform.localPosition.y, transform.localPosition.z);
                    SpringPanel.Begin(mPanel.gameObject, pos, 13f).strength = 8f;
                }               
                return delta.x > 0;
            }
        ...
        {
            if (transform.localPosition.y > mCanDragLimitPos.y)
            {
                float yOffset = transform.localPosition.y - mCanDragLimitPos.y;
                if (yOffset > CirticalValue2CorrectedPos)
                {
                    Vector3 pos = new Vector3(transform.localPosition.x, mCanDragLimitPos.y, transform.localPosition.z);
                    SpringPanel.Begin(mPanel.gameObject, pos, 13f).strength = 8f;
                }
                return delta.y < 0;
            }
